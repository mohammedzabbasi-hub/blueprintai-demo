...
export default function AdBriefs() {
  ...
  useEffect(() => {
    async function load() {
      const endpoint = demo
        ? `${API_BASE}/briefs?shop_id=${shopId}`
        : `${API_BASE}/personalized/briefs?shop_id=${shopId}`;
      ...
      <div className="space-y-6">
        {briefs.map((brief, index) => (
          <div key={brief.id || index} className="rounded-3xl border border-slate-800 bg-[#0b1220] p-7">
            <h2 className="text-3xl font-black">{brief.title || "Ad Brief"}</h2>
            <p className="text-slate-400 mt-4">{brief.description || brief.content || brief.summary || ""}</p>
          </div>
        ))}
      </div>
    </div>
